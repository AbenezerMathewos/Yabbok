...
interface BackgroundGeometricProps {
    className?: string;
    color1?: string;
    color2?: string;
    speed?: number;
...
export function BackgroundGeometric({
    color1,
    color2,
    speed = 1,
    className,
}: BackgroundGeometricProps) {
    const { resolvedTheme } = useTheme();
    
    // Dark mode defaults: Deep Slate to Subtle Gold
    const defaultColor1 = resolvedTheme === 'dark' ? "#020617" : "#f8fafc"; 
    const defaultColor2 = resolvedTheme === 'dark' ? "#1e293b" : "#e2e8f0";

    return (
        <div className={cn("fixed inset-0 w-full h-full z-[-1] pointer-events-none opacity-80", className)}>
            <Canvas
            ...
            >
                <GradientPlane 
                    color1={color1 || defaultColor1} 
                    color2={color2 || defaultColor2} 
                    speed={speed} 
                />
            </Canvas>
        </div>
